...
"""corpusbuilder.py: scrapes submission headlines from tech news
    subreddits and create a corpus from which Markov chains can be
    generated.
    """
...
def age_limit_to_seconds(my_int=120):
    ...
    if type(my_int) != int:
        try:
            ...
            logger.error('reverting to default age limit of 120 days')
            my_int = 120
    my_int = 120 if my_int <= 0 else my_int
    return 60 * 60 * 24 * my_int
...
def clean_corpus():
    ...
    for file in paths:
        ...
        now = time.time()
        limit = now - age_limit_to_seconds(corpus_age_limit)
        if m < limit:
            os.remove(file)
...
def clean_logs():
    ...
    for file in paths:
        m = os.path.getctime(file)  # could try os.path.getmtime()
        now = time.time()
        limit = now - age_limit_to_seconds(checked_logs_age_limit)
        if m < limit:
            os.remove(file)
            os.touch(file)
